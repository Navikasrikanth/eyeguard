...
const POSTURE_WARNING_SCORE_THRESHOLD = 0.22;
const POSTURE_WARNING_CLEAR_THRESHOLD = 0.1;
const POSTURE_STRONG_REASON_SCORE_THRESHOLD = 0.12;
const STRONG_POSTURE_REASONS = new Set([
  "slumped forward",
  "slumped shoulders / compressed neck",
  "uneven shoulders",
  "head tilt"
]);
...
export function useMonitoringEngine({
  ...
  useEffect(() => {
    ...
    async function start() {
      try {
        ...
        setHelperMessage("Hold your most upright neutral posture for about 2 seconds to calibrate EyeGuard.");
          ...
          if (landmarks) {
            ...
            if (!baselineRef.current) {
              ...
              if (calibrationSamplesRef.current.length >= CALIBRATION_SAMPLE_TARGET) {
                ...
                setHelperMessage(
                  poseLandmarker
                    ? "Posture baseline captured from your upright neutral pose. Recalibrate after changing your chair, desk height, or monitor angle."
                    : "Baseline captured from head cues using your upright neutral pose. Recalibrate after changing your chair, desk height, or monitor angle."
                );
            ...
            const hasStrongReason = assessment.reasons.some((reason) => STRONG_POSTURE_REASONS.has(reason));
            const shouldWarn =
              assessment.reasons.length > 0 &&
              (metrics.score >= POSTURE_WARNING_SCORE_THRESHOLD ||
                (hasStrongReason && metrics.score >= POSTURE_STRONG_REASON_SCORE_THRESHOLD));

            const previousState = postureStateRef.current;
            let nextState: MonitoringState["postureState"] = previousState;
            if (previousState !== "warning" && shouldWarn) {
              nextState = "warning";
            } else if (previousState === "warning" && metrics.score < POSTURE_WARNING_CLEAR_THRESHOLD) {
              nextState = "good";
            } else if (previousState === "unknown") {
              nextState = shouldWarn ? "warning" : "good";
            }
  ...
  return {
    ...
    recalibrate() {
      setCalibrationState("needed");
      setCalibrationProgress(0);
      setHelperMessage("Recalibrating posture baseline. Hold your most upright neutral posture for about 2 seconds.");
