...
export function assessPosture(
  ...
  if (rollDeltaDegrees > 7 * toleranceScale) {
    pushUnique(reasons, "head tilt");
  }

  if (verticalDrift > 0.032 * toleranceScale) {
    pushUnique(reasons, "head dropped below neutral");
  } else if (verticalDrift < -0.04 * toleranceScale) {
    pushUnique(reasons, "chin lifted above neutral");
  }

  if (centerDelta > 0.05 * toleranceScale) {
    pushUnique(reasons, "head shifted off-center");
  }

  if (leanDelta > 0.11 * toleranceScale) {
    pushUnique(reasons, current.leanRatio > baseline.leanRatio ? "leaning toward screen" : "leaning away from screen");
  }

  if (shoulderTracked && shoulderRollDeltaDegrees > 6 * toleranceScale) {
    pushUnique(reasons, "uneven shoulders");
  }

  if (shoulderTracked && shoulderNeckDelta > 0.06 * toleranceScale) {
    pushUnique(reasons, "slumped shoulders / compressed neck");
  }

  const slumpComposite =
    Math.max(0, verticalDrift) + shoulderNeckDelta + (current.leanRatio > baseline.leanRatio ? leanDelta * 0.65 : 0);
  if (slumpComposite > 0.08 * toleranceScale) {
    pushUnique(reasons, "slumped forward");
  }

  const score =
    0.22 * contribution(rollDeltaDegrees, 5.4 * toleranceScale) +
    0.24 * contribution(verticalDelta, 0.028 * toleranceScale) +
    0.1 * contribution(centerDelta, 0.034 * toleranceScale) +
    0.14 * contribution(leanDelta, 0.075 * toleranceScale) +
    0.12 * contribution(shoulderRollDeltaDegrees, 4.8 * toleranceScale) +
    0.24 * contribution(shoulderNeckDelta, 0.04 * toleranceScale) +
    (reasons.length >= 2 ? 0.18 : 0) +
    (reasons.includes("slumped forward") ? 0.08 : 0);
