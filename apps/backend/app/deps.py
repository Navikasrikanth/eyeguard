...
#check if user is authenticated and return user id or else raises 401 error
def get_current_user_id(authorization: str = Header(default="")) -> int:
    ...
    except jwt.PyJWTError as error:#catch jwt errors and raises error
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Invalid access token.") from error

    if payload.get("type") != "access":#check if access token
